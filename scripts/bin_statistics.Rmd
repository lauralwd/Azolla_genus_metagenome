...
note to self: actually I'm only interested in the no_support counts, so I can melt all taxnomies in a long table, filter for no support and make a "fail" count per taxonomy level... 

```{r}
attach(bincollections)
# set the basic data and aesthetic
bincol_plot <- ggplot(data = bincollections)
bincol_plot <- bincol_plot + geom_bar(aes(x=order,
                                          fill=order))


# facets
bincol_plot <- bincol_plot + facet_grid(Collection ~ Sample)
bincol_plot <- bincol_plot + scale_fill_discrete()
